...
public:
    ...
    double SalesOfMonth(int year, int month)
    {
        ...
        if (isValidDate(year, month, 1))
        {
            ...
            if (month <= currentYear.months.size())
            {
                ...
                std::cout << " The total sales for this month  " << year << " / " << month << " is :  " << totalSales << endl;
                ;
                return totalSales;
            }
            else
    ...
    double TotalSalesBetweenDates(int startYear, int startMonth, int startDay, int endYear, int endMonth, int endDay)
    {
        ...
        cout << " The total Sales from " << startYear << "/" << startMonth << "/" << startDay << " to " << endYear << "/" << endMonth << "/" << endDay << " is : " << endl;
...
private:
    ...
    int getBalanceFactor(Restaurant *root)
    {
        if (root == nullptr)
        {
            return 0;
    ...
    void updateHeight(Restaurant *root)
    {
        if (root == nullptr)
        {
            return;
    ...
    Restaurant *rotateLeft(Restaurant *node)
    {
        Restaurant *newRoot = node->right;
    ...
    Restaurant *rotateRight(Restaurant *node)
    {
        Restaurant *newRoot = node->left;
    ...
    void insert(Restaurant *&root, Restaurant *t)
    {
        ...
        if (balanceFactor > 1 && t->id < root->left->id)
        {
            root = rotateRight(root);
        }

        // Right-Right case: Perform a left rotation
        if (balanceFactor < -1 && t->id > root->right->id)
        {
    ...
    Restaurant *removeRestaurant(Restaurant *root, long long id)
    {
        ...
        else
        {
            ...
            {
                Restaurant *temp = (root->left != nullptr) ? root->left : root->right;
                if (temp == nullptr)
                {
                    temp = root;
                    root = nullptr;
    ...
    Restaurant *findMin(Restaurant *node)
    {
    ...
    void searchIncountry(Restaurant *root, int year) const
    {
        if (root != nullptr)
        {
            ...
            std::cout << "Restaurant: " << root->name << ", ID: "
                      << " total Sales in given Year  : " << root->id << root->sales.SalesOfYear(year) << std::endl;
    ...
    void searchIncountry(Restaurant *root, int year, int month) const
    {
        if (root != nullptr)
        {
            ...
            std::cout << "Restaurant: " << root->name << ", ID: "
                      << " total Sales in given Year  : " << root->id << root->sales.SalesOfMonth(year, month) << std::endl;
    ...
    void searchIncountry(Restaurant *root, int syear, int smonth, int sday, int eyear, int emonth, int eday) const
    {
        if (root != nullptr)
        {
            ...
            std::cout << "Restaurant: " << root->name << ", ID: "
                      << " total Sales in given Year  : " << root->id << root->sales.TotalSalesBetweenDates(syear, smonth, sday, eyear, emonth, eday) << std::endl;
...
public:
        ...
        while (std::getline(infile, line))
        {
            ...
            try
            {
                ...
                if (id == root->id && iyear == year && imonth == month)
                {
                    ...
                    // cout<< root->sales.displayMonthlySalesForCuisine(year, month, type)<<endl;
                    if (transformedType == "algerian")
                    {
                        // cout<<std::stod(Algerian_Rate) <<endl ;
    ...
    void theWinnersOfTheNationalPrize(int month, int year)
    {
        ...
        cout << "the name of restaurant which its Algerian cuisine won is: ";
        cout << theFiveWinners[0] << endl;
        cout << "the name of restaurant which its Syrian cuisine won is: ";
        cout << theFiveWinners[1] << endl;
        cout << "the name of restaurant which its Chinese cuisine won is: ";
        cout << theFiveWinners[2] << endl;
        cout << "the name of restaurant which its Indian cuisine won is: ";
        cout << theFiveWinners[3] << endl;
        cout << "the name of restaurant which its European cuisine won is: ";
    ...
    {
        cout << " alll resturant in country with year" << year << endl;
        searchIncountry(root, year);
    }

    void searchCountry(int year, int month)
    {
        cout << " all resturant in country with year" << year << " and month : " << month << endl;
        searchIncountry(root, year, month);
    }
    void searchCountry(int syear, int smonth, int sday, int eyear, int emonth, int eday)
    {
        cout << " all  resturant in country from  year" << syear << " /" << smonth << "/" << sday << " to : " << eyear << "/" << emonth << "/" << eday << endl;
...
int main()
{
    ...
    ofstream testfile("C:\\xampp\\htdocs\\Bouzira Maroua\\code\\data.csv", ios::app);
    testfile.close();
    ...
    std::string restaurantFilename = "C:\\xampp\\htdocs\\Bouzira Maroua\\code\\data.csv";
    std::string costsSalesFilename =  "C:\\xampp\\htdocs\\Bouzira Maroua\\code\\dataforSalesCosts.csv";
